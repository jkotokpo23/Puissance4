...
public class GUI extends JFrame {
	...
	public JLayeredPane layeredGameBoard;
	public JButton[] buttons;
	public JLabel turnLabel;
	public AI ai;
	public AI assistantIa;
	...
	public GUI() {
		...
		assistantIa = new MinimaxAlphaBetaPruningAI(gameParameters.getAi2MaxDepth(), Constants.P2);
	}
	...
	public GUI(String title) {
		...
		startButton = new JButton("Reprendre");
		redoButton = new JButton(">>");
		resetButton = new JButton("Reinitialiser");
		bestPlay = new JButton("Meilleur colonne");
		...
		bestPlay.addActionListener(e -> {
			JOptionPane.showMessageDialog(compMainWindowContents, "La meilleur position est la colonne X");
		});
		...
		turnLabel = new JLabel("Tour: " + board.getTurn());
	...
	public void startNewGame() {
		...
		super.add(tools, BorderLayout.PAGE_END);
		turnLabel.setText("Tour: " + board.getTurn());
		...
		System.out.println("Tour: " + board.getTurn());
	...
	private void addMenus() {
		...
		undoItem = new JMenuItem("Retour    Ctrl+Z");
		redoItem = new JMenuItem("Refaire    Ctrl+Y");
		JMenuItem saveGameItem = new JMenuItem("Sauvegarder");
		JMenuItem restoreSavedGameItem = new JMenuItem("Dernière partie");
		JMenuItem insertCheckerItem = new JMenuItem("Inserer un Checker");
		JMenuItem exportToGifItem = new JMenuItem("Capture d'écran");
		JMenuItem settingsItem = new JMenuItem("Paramètres");
		JMenuItem exitItem = new JMenuItem("Quitter");

		JMenu helpMenu = new JMenu("Aide");
		JMenuItem howToPlayItem = new JMenuItem("Règles");
		...
		restoreSavedGameItem.addActionListener(e -> {
			restoreSavedGame();
			System.out.println("Dernière partie restaurée !");
		});
		...
		howToPlayItem.addActionListener(e -> JOptionPane.showMessageDialog(
				this,
				"Cliquer sur les boutons ou appuyer sur 1-" + board.getNumOfColumns() + "sur le clavier pour inserer une nouvelle pièce."
						+ "\nPour gagner " + board.getCheckersInARow() + " piece dans la colonne, horizontalement, verticalement or diagonalement.",
				"Règles",
		...
		aboutItem.addActionListener(e -> {
			JLabel label = new JLabel(
					"<html>&copy; Projet d'intégration <br>"
							+ "Version " + GuiConstants.VERSION + "</html>"
			);
			JOptionPane.showMessageDialog(this, label, "A propos", JOptionPane.INFORMATION_MESSAGE);
		});
	...
	public void saveGame() {
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("lastPart.txt"));) {
	...
	public void restoreSavedGame() {
		try (BufferedReader br = new BufferedReader(new FileReader("lastPart.txt"));) {
			String line;
	...
	public void exportToGif() {
		...
		try {
			...
			System.out.println("Opération validée .gif!");
		} catch (Exception ex) {
			System.err.println("Erreur .gif !");
			System.err.flush();
		}
	}
	...
	public JLayeredPane initializeLayeredBoard() {
		...
		if (gameParameters.getCheckersInARow() == CONNECT_4_CHECKERS_IN_A_ROW) {
			...
			imageBoard = new ImageIcon(this.getClass().getResource("images/Board4.png"));
	...
	private void undo() {
		if (!undoBoards.isEmpty()) {
			try {
				...
				System.err.println("Vous n'avez pas encore joué!");
				System.err.flush();
			}
	...
	private void redo() {
		if (!redoBoards.isEmpty()) {
			...
			try {
				...
				System.err.println("Aucun mouvement à refaire!");
				System.err.flush();
			}
	...
	public void placeChecker(Color color, int row, int col) {
		...
		int xOffset = 75 * col;
		int yOffset = 75 * row;
		ImageIcon checkerIcon = new ImageIcon(this.getClass().getResource("images/" + colorString + ".png"));
	...
	public void gameOver() {
		...
		if (board.getWinner() == Constants.P1) {
			if (gameParameters.getGameMode() == GameMode.HUMAN_VS_AI)
				...
				choice = JOptionPane.showConfirmDialog(
						this,
						"Joueur 1 a gagné! Voulez vous rejouer?",
						"Game Over",
						JOptionPane.YES_NO_OPTION
				);
			else if (gameParameters.getGameMode() == GameMode.AI_VS_AI)
				choice = JOptionPane.showConfirmDialog(
						this,
						"Minimax AI 1 a gagné! Voulez vous rejouer?",
						"Game Over",
						JOptionPane.YES_NO_OPTION
				);
			...
			if (gameParameters.getGameMode() == GameMode.HUMAN_VS_AI)
				choice = JOptionPane.showConfirmDialog(
						this,
						"Computer AI a gagné! Voulez vous rejouer?",
						"Game Over",
						JOptionPane.YES_NO_OPTION
				);
			else if (gameParameters.getGameMode() == GameMode.HUMAN_VS_HUMAN)
				choice = JOptionPane.showConfirmDialog(
						this,
						"Player 2 a gagné! Voulez vous rejouer?",
						"Game Over",
						JOptionPane.YES_NO_OPTION
				);
			else if (gameParameters.getGameMode() == GameMode.AI_VS_AI)
				choice = JOptionPane.showConfirmDialog(
						this,
						"Minimax AI 2 a gagné! Voulez vous rejouer?",
						"Game Over",
						JOptionPane.YES_NO_OPTION
				);
