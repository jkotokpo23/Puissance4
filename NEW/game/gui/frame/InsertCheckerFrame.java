...
public class InsertCheckerFrame extends JFrame {
	...
	public InsertCheckerFrame(GUI gui) {
		...
		JLabel rowLabel = new JLabel("Ligne: ");
		JLabel columnLabel = new JLabel("Colonne: ");
		...
		player_drop_down.addItem("Joueur 1");
		player_drop_down.addItem("Joueur 2");
			...
			if (ev.getSource() == cancel) {
				...
				try {
					...
					if (gui.board.getGameBoard()[row][column] == Constants.EMPTY) {
						...
						gui.turnLabel.setText("Tour: " + gui.board.getTurn());
					}
					...
					System.out.println("Pièce inserée!");
